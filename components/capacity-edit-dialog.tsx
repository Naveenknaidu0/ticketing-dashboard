...
interface CapacityEditDialogProps {
  isOpen: boolean
  profile: any
  onClose: () => void
  onSave: (updatedProfile: any, isDraft: boolean) => void
}

const PROFILE_ROLES = ['l1-agent', 'l2-specialist', 'l3-expert', 'custom']

export function CapacityEditDialog({ isOpen, profile, onClose, onSave }: CapacityEditDialogProps) {
  const [step, setStep] = useState(1)
  const [hasChanges, setHasChanges] = useState(false)
  const [formData, setFormData] = useState(profile ? {
    name: profile.name || '',
    role: profile.role || 'l1-agent',
    description: profile.description || '',
    maxOpenTickets: profile.maxOpenTickets || 15,
    maxCritical: profile.maxCritical || 2,
    maxHigh: profile.maxHigh || 5,
    maxSlaRisk: profile.maxSlaRisk || 3,
    maxDailyAssignments: profile.maxDailyAssignments || 50,
    maxConcurrent: profile.maxConcurrent || 3,
  } : {})
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-8 w-full max-w-2xl max-h-96 overflow-y-auto">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold" style={{ color: '#0D3133' }}>Edit Capacity Profile</h2>
